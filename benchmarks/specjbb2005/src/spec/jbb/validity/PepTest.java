 ...
 * reserved. This source code is provided as is, without any express or implied
 * warranty.
 */
...
class PepTest {
  ...
  private final static boolean deliberateErrorForTest = false;
 ...
 /*
     * subClass a[] = new subClass[2]; ((superClass[])a)[1] = new superClass();
     */
 ...
 */
  ...
  int deepRecursion(int n, int sum) {
    ...
    return deepRecursion(n - 1, n + sum);  /*
                                             * Hopefully javac won't elim. tail
                                             * recursion.
                                             */
  ...
  String testGC() {
    ...
    /*
     * Unfortunately we cannot count on being able to get system properties in
     * an applet, so skip this test. wnb 2/13/98
...
/*
 * This is more properly a QA test. Remove per (osgjava-363)
...
// (new FloatingPointCheck()).run (spec.harness.Context.getSpeed());
    ...
    if (gotError){
      ...
      // out.println("****** PepTest found an error ******");

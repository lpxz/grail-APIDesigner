...
public class MethodNode extends AbstractNode {
	...
	private String	callsite;	// HK: this is important. Remeber that a calling
								// context is a (callsite, methodsignature) pair

	private String	mem_str;	// not important
	private String	methodSig;	// the methodsignature

	// these information is not important in violation detection, but is
	// important for the later Petri net construction.
	// make sure that, the context information you stored allow you to construct
	// the Petri net without any ambiguity

	public MethodNode(int mem, long tid, TYPE type, boolean isPrivate,
			String callsiteByHK) {
		...
		// }
		// else {
		// this.appSTE =process2(Thread.currentThread().getStackTrace(),
		// msubsig);
		// }
		...
		// }
		// this.appSTE = new StackTraceElement(tmp.getClassName(),
		// tmp.getMethodName(), tmp.getFileName(), tmp.getLineNumber());
	...
	// private StackTraceElement_lpxz process2(StackTraceElement[] stackTrace,
	// String msubsig) {
	// int lateIndex = 0;
	// int hisParentIndex =0;
	//
	// for(int i=0; i<stackTrace.length; i++)// late is placed at the starting
	// region...
	// {
	...
	// // StackTraceElement parent = findFirstInstrumentedParent(stackTrace,
	// hisParentIndex);//stackTrace[hisParentIndex];
	...
	// }
	//
	// return new StackTraceElement_lpxz(tmp.getClassName(),
	// tmp.getMethodName(),parentFileName, parentLineNo, parentLineInvoke,
	// msubsig);
	...
	// if(ele.toString().startsWith("edu.hkust.clap.") ||
	// ele.toString().startsWith("java.lang.reflect.") ||
	// ele.toString().startsWith("sun.reflect."))
	// {
	// continue;
	// }
	// else {
	...
	// }
	// // System.out.println("" );
	// for(int k =earlyIndex; k>=lateIndex; k--)// from ending Index to starting
	// index
	// {

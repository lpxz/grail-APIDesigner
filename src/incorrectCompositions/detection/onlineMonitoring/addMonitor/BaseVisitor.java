...
public class BaseVisitor {
	...
	protected BaseVisitor					nextVisitor;
	...
	public BaseVisitor(BaseVisitor nextVisitor) {
	...
	public void visitStmtThrow(SootMethod sm, Chain units, ThrowStmt throwStmt) {
		...
		// check condition. return-like throwing.
		if (!returnLikeThrow(sm, units, throwStmt)) {
			// do nothing, it is not a return-like throw. treat it as a common stmt.
		} else {
			BaseVisitor.addCallMethodExit(sm, units, throwStmt); // 1 common method
			// 2,3,4:
			if (JTPTransformer.isMethodMain) {
				// 2 main method exit, safely ignore
			} else if (JTPTransformer.isMethodRunnable) {
				BaseVisitor.addCallRunMethodExitBefore(sm, units, throwStmt);// 3 run method exit
			}

			if (JTPTransformer.isMethodSynchronized)// 4 synchronized method.
			{
				String sig;
				Value memory;
				Value base;
				if (sm.isStatic()) {
					...
					BaseVisitor.addCallAccessSyncObjInstance(sm, units,
							throwStmt, "exitMonitorBefore",
							StringConstant.v(sig), memory, true);
				} else {
					...
					if (firstStmt instanceof IdentityStmt) {
						...
						BaseVisitor.addCallAccessSyncObjInstance(sm, units,
								throwStmt, "exitMonitorBefore", base, memory,
								true);
					}
				}
			}
	...
	private boolean returnLikeThrow(SootMethod sm, Chain units,
			ThrowStmt throwStmt) {
			...
			for (Unit child : childrenChain) {
				if (!visited.contains(child)) {
					...
					if (child instanceof ReturnStmt
							|| child instanceof ReturnVoidStmt) {
						System.out
								.println("the throw is not a return-like throw, it is like a common stmt which reaches the return finally.");
						return false;
	...
	public void visitStmtReturnVoid(SootMethod sm, Chain units,
		...
		BaseVisitor.addCallMethodExit(sm, units, returnVoidStmt); // 1 common method
		// 2, 3, 4:
		if (JTPTransformer.isMethodMain) {
			// 2 main method exit, safely ignore it. lpxz
		} else if (JTPTransformer.isMethodRunnable) {
			BaseVisitor.addCallRunMethodExitBefore(sm, units, returnVoidStmt); // 3 run method exit.
		}
		...
		if (JTPTransformer.isMethodSynchronized) // 4 synchronized method
		{
			...
			Value base;
			if (sm.isStatic()) {
				sig = sm.getDeclaringClass().getName() + ".STATIC.";//do not change the constant! lpxz
				memory = StringConstant.v(sig);
				BaseVisitor.addCallAccessSyncObjInstance(sm, units,
						returnVoidStmt, "exitMonitorBefore",
						StringConstant.v(sig), memory, true);
	...
	public void visitStmtReturn(SootMethod sm, Chain units,
		...
		BaseVisitor.addCallMethodExit(sm, units, returnStmt); // 1 common method
		// 2,3,4:
		if (JTPTransformer.isMethodMain) {
			// 2 main method exit, safely ignore it, lpxz	
		} else if (JTPTransformer.isMethodRunnable) {
			BaseVisitor.addCallRunMethodExitBefore(sm, units, returnStmt); // 3 run method exit.
		}
		...
		if (JTPTransformer.isMethodSynchronized) // 4 synchronized method.
		{
			...
			if (sm.isStatic()) {
				sig = sm.getDeclaringClass().getName() + ".STATIC.";//+"."+invokeExpr.getMethod().getName();
				memory = StringConstant.v(sig);
				BaseVisitor.addCallAccessSyncObjInstance(sm, units, returnStmt,
						"exitMonitorBefore", StringConstant.v(sig), memory,
						true);
			} else {
				sig = sm.getDeclaringClass().getName();
				memory = StringConstant.v(sig);
				...
				if (firstStmt instanceof IdentityStmt) {
					base = ((IdentityStmt) firstStmt).getLeftOp();
					BaseVisitor
							.addCallAccessSyncObjInstance(sm, units,
									returnStmt, "exitMonitorBefore", base,
									memory, true);
				}
			}
	...
	public void addCallReCrashWith(SootMethod sm, Chain units,
		...
		args.addLast(e);
		SootMethodRef mr = Scene.v().getMethod(Constants.CATCH_EXCEPTION_SIG)
				.makeRef();
		units.insertAfter(
	...
	public void addCallCaredByClap(SootMethod sm, Chain units,
			AssignStmt assignStmt) {
		LinkedList args = new LinkedList();

		String name = assignStmt.getRightOp().toString()
				.replaceFirst(Constants.CARE_ANNOTATION, "");
		...
		SootMethodRef mr = Scene.v().getMethod(Constants.CARED_BY_CLAP_SIG)
				.makeRef();
		units.insertAfter(
				Jimple.v().newInvokeStmt(
						Jimple.v().newStaticInvokeExpr(mr, args)), s1);
	...
	public static Stmt getThreadIdentityStmt(Chain units, String methodname) {
		...
		SootMethodRef mr = Scene
				...
				.getMethod(
						"<"
								...
								+ ": void "
								+ methodName
								+ "(java.lang.Object,int,long,int,java.lang.String,java.lang.String)>")
				.makeRef();
		if (before) {
			units.insertBefore(
					Jimple.v().newInvokeStmt(
		...
		SootMethodRef mr = Scene
				...
				.getMethod(
						"<"
								...
								+ ": void "
								+ methodName
								+ "(int,long,int,java.lang.String,java.lang.String)>")
				.makeRef();
		if (before) {
			units.insertBefore(
					Jimple.v().newInvokeStmt(
	...
	public static void addCallSpecialMonitorEntryInstanceAfterClassLocal(
			SootMethod sm, Chain units, String methodName, Value base, Value v,
			boolean before) {
	...
	public static Local getMethodThreadId(SootMethod sm) {
		...
		if (tid == null) {
			tid = BaseVisitor.addLocalThreadId(bb);
		}
	...
	private static Local locateLocalThreadId(Body bb) {
		...
		while (it.hasNext()) {
			...
			if (unit.containsInvokeExpr()
					&& unit.toString()
							.contains(
									"<java.lang.Thread: java.lang.Thread currentThread()>")) { // find possible
				Stmt nextOne = (Stmt) units.getSuccOf(unit);
				if (nextOne.containsInvokeExpr()
						&& nextOne.toString().contains(
					...
					if (nextOne instanceof AssignStmt) {
						...
						if (tid.getType() instanceof LongType) {
							...
							throw new RuntimeException(
									"how can you fail at this place, even"
											+ tid.getType());
						}
					}
				}
	...
	public static void addCallCatchException(Body body) {
		...
		while (stmtIt.hasNext()) {
			...
			if (s instanceof IdentityStmt)
				 ...
				 * if(s.toString().contains(Parameters.CRASH_ANNOTATION)) { Stmt
				 * s1=(Stmt) units.getSuccOf(s); Value e
				 * =s1.getInvokeExpr().getArg(0); LinkedList args = new
				 * LinkedList(); args.addLast(e); SootMethodRef mr =
				 * Scene.v().getMethod
				 * (Parameters.CATCH_EXCEPTION_SIG).makeRef();
				 * units.insertAfter(
				 * Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(mr,
				 * args)),s1); } }
				 */
	...
	public static void addCallMonitorEntry(Body body) {
		...
		if (appMethod.isStatic()) {
			// SHOULD BE the lock on "appClass.class"
			//	        $r2 = class "org/exolab/jms/common/uuid/Clock";
			//	        r1 = $r2;

			sig = appClass.getName() + ".STATIC.";//+"."+invokeExpr.getMethod().getName();
			memory = StringConstant.v(sig);
			//  Visitor.addCallSpecialMonitorEntry(appMethod,units, "enterMonitorAfter", memory, false);	// wrong version in pecan! leads to many false bugs!

			//	        Chain<Local> locals =appMethod.getActiveBody().getLocals();
			//	        Local classlocal = null;
			//	        for(Local local : locals)
			//	        {
			//	        	if(local.getName().equals("classlocallpxz"))
			//	        	{
			//	        		classlocal =local;
			//	        	}
			//	        }
			//	        if(classlocal ==null) throw new RuntimeException("I have initialized in JTPTransformer.class");
			BaseVisitor.addCallSpecialMonitorEntryInstanceAfterClassLocal(
					appMethod, units, "enterMonitorAfter",
					StringConstant.v(sig), memory, false);
			System.out.println(appMethod.getActiveBody());
		} else {
			...
			if (firstStmt instanceof IdentityStmt) {
				base = ((IdentityStmt) firstStmt).getLeftOp();
				BaseVisitor.addCallSpecialMonitorEntryInstance(appMethod,
	...
	// by default, we treat all methods as nonprivate, no need to distinguish private and nonprivate.
	public static void addCallMethodEntry(SootMethod sm, Chain units) {

		String methodname = sm.getDeclaringClass().getName() + "."
				+ sm.getName();
		LinkedList args = new LinkedList();
		...
		if (JTPTransformer.isMethodStaticNonPara)
			s = (Stmt) units.getSuccOf(s);

		SootMethodRef mr_nonprivate = Scene
				.v()
				.getMethod(
						"<" + observerClass + ": void "
								+ "enterCommonMethodAfter"
								+ "(int,long,java.lang.String)>").makeRef();

		units.insertAfter(
				Jimple.v().newInvokeStmt(
						Jimple.v().newStaticInvokeExpr(mr_nonprivate, args)), s);
	}

	public static void addCallMethodExit(SootMethod sm, Chain units, Stmt s) {
